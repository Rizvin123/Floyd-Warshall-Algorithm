...
def recursive_floyd_warshall(outer_loop:int, middle_loop:int, inner_loop:int):
        ...
        Stimulates three nested loops using recursion only.
        """

        # Base case: finished all outer iterations
        if outer_loop >= MAX_LENGTH:
            return
        
        # If finished inner loop, move to next middle loop
        if inner_loop >= MAX_LENGTH:
            recursive_floyd_warshall(outer_loop, middle_loop + 1, MIN_LEVEL)
            return
        
        # If finished middle loop, move to next outer loop
        ...
        if middle_loop == inner_loop:
            ...
            if GRAPH[middle_loop][outer_loop] != NO_PATH and \
